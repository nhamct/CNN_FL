...
FORMULA = ['barinel', 'dstar2', 'jaccard', 'muse', 'ochiai', 'opt2', 'tarantula','cnn']
...
PROJECTS = ['Closure', 'Chart']
PROJECT_BUGS = [
    [str(x) for x in range(1, 3)],
...
def calculate_accuracy(input_dir, bug_dir, top_n):
    ...
    for project, bugs in zip(PROJECTS, PROJECT_BUGS):
        for bug in bugs:
            for formula in FORMULA: #NHamCT
                try:
                    input_file = os.path.join(input_dir, '%s-%s-%s-sorted-susp' % (project, bug,formula))#NhamCT
                    ...
                    #accuracy = len(reweighted_lines.intersection(buggy_lines))
